...
export default function PdfToSlidesUploader({
    ...
    const setPct = (p: number) => setProgress(Math.max(0, Math.min(100, Math.round(p))));
    ...
    const handleUpload = useCallback(async () => {
        try {
            ...
            // 진행 퍼센트 배분: 분석/정리 20% + 페이지 루프 80%
            const base = 20;
            const perPage = total ? 80 / total : 80;
            setPct(20);
            ...
            for (let i = 1; i <= total; i++) {
                if (cancelRef.current) { pushLog("사용자에 의해 취소됨"); break; }
