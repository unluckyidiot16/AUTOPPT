...
export default function TeacherPage() {
    ...
    const currentSlideIndex = orderedSlides.findIndex((s) => s.slide === currSlide);
    const currentSlide = orderedSlides[currentSlideIndex];
    const stepsOfCurrent = currentSlide?.steps ?? [];
    ...
    // ----- 슬롯 목록 -----
    const [slots, setSlots] = useState<SlotRow[]>(() => Array.from({ length: 6 }, (_, i) => ({ slot: i + 1, deck_id: null })));
    useEffect(() => {
        (async () => {
            if (!roomCode) return;
    ...
    useEffect(() => {
        ...
        (async () => {
            if (!currentDeckId) {
                DBG.info("No currentDeckId, clearing deckFileUrl");
                setDeckFileUrl(null);
                return;
            }
            if (!roomId) {
                DBG.info("No roomId yet, waiting...");
                return;
            }

            DBG.info(`Loading PDF for deck: ${currentDeckId}, room: ${roomId}`);

            // 먼저 room_decks 조인으로 시도
            const tryPick = async () => {
                ...
                if (error) {
                    DBG.err("Error fetching from room_decks:", error);
                    return null;
                }

                const fileKey = (rd as any)?.decks?.file_key;
                DBG.info("room_decks result:", fileKey);
                return fileKey ?? null;
            };
            ...
            for (let i = 0; i < 18 && !fk; i++) {
                fk = await tryPick();
                if (!fk && i < 17) {
                    DBG.info(`Retry ${i + 1}/18 - waiting 200ms...`);
                    await new Promise(r => setTimeout(r, 200));
                }
            }

            // 그래도 못 찾으면 decks 테이블 직접 조회
            if (!fk) {
                DBG.info("Trying direct decks table query...");
                ...
                if (error) {
                    DBG.err("Error fetching from decks:", error);
                } else {
                    fk = (d2 as any)?.file_key ?? null;
                    DBG.info("decks table result:", fk);
                }
            }
            ...
            if (fk) {
                const url = getPublicUrl(fk);
                DBG.ok(`Setting deckFileUrl: ${url}`);
                setDeckFileUrl(url);
            } else {
                DBG.err("No file_key found for deck:", currentDeckId);
                setDeckFileUrl(null);
            }
        })();
        ...
        if (currentDeckId) {
            const ch = supabase.channel(`decks:${currentDeckId}`)
                .on("postgres_changes", {
                    ...
                    schema: "public",
                    table: "decks",
                    filter: `id=eq.${currentDeckId}`
                }, (ev: any) => {
                    DBG.info("Deck update event:", ev);
                    const fk = ev.new?.file_key;
                    if (fk) {
                        const url = getPublicUrl(fk);
                        DBG.ok(`Realtime update - Setting deckFileUrl: ${url}`);
                        setDeckFileUrl(url);
                    }
                })
                .subscribe();
    ...
    // ----- 교사 화면 알림 -----
    const [notify, setNotify] = useState<{ key: string; studentId: string; msg: string } | null>(null);
    const [connectedStu, setConnectedStu] = useState<string[]>([]);
    const onTeacherNotify = (studentId: string, msg: string) => {
        const k = `${studentId}-${Date.now()}`;
        setNotify({ key: k, studentId, msg });
        setTimeout(() => setNotify((n) => (n?.key === k ? null : n)), 3000);
        setConnectedStu((prev) => prev.includes(studentId) ? prev : [...prev, studentId]);
    };
    useEffect(() => { if (lastMessage?.type === "hello" && lastMessage.role === "student") onTeacherNotify("학생", "접속함"); }, [lastMessage]);
    ...
    async function uploadPdfForSlot(slot: number) {
        ...
        input.onchange = async () => {
            ...
            try {
                const title = toSlug(file.name);
                const extId = `pdf_${title}_${Date.now()}`;
                const fileKey = `${roomCode}/slot${slot}_${extId}.pdf`;

                setUploadPct(10, "파일 준비 중...");
                const { data: upData, error: upErr } = await supabase.storage.from("presentations").upload(fileKey, file, {
                    upsert: true,
                    contentType: "application/pdf",
                });
                if (upErr) throw upErr;

                setUploadPct(50, "데이터베이스 업데이트 중...");
                ...
                setUploadPct(70, "슬롯에 배정 중...");
                const { error: assignErr } = await rpc("assign_room_deck_by_id", {
                    p_code: roomCode,
                    p_slot: slot,
                    p_deck_id: deck.id,
                });
                ...
                setUploadPct(85, "현재 교시로 설정 중...");
                const { error: setErr } = await rpc("set_room_deck", { p_code: roomCode, p_slot: slot });
                ...
                // 미리보기 URL 설정
                const previewUrl = getPublicUrl(fileKey);
                setUploadDlg((u) => ({ ...u, previewUrl }));
                ...
                toast.show(`슬롯 ${slot}에 PDF 업로드 완료`);
            } catch (e) {
                console.error(e);
                clearInterval(timer);
        ...
        <div className="panel" style={{ padding: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                <div style={{ fontSize: 12, opacity: 0.7 }}>
                    슬라이드 {currSlide} / 스텝 {currStep}
                    {currentDeckId ? ` (덱: ${currentDeckId.slice(0, 8)}...)` : " (덱 미선택)"}
                </div>
            ...
            <div style={{ display: "grid", placeItems: "center" }}>
                {deckFileUrl ? (
                    <div className="pdf-stage" style={{ width: "100%" }}>
                        <PdfViewer fileUrl={deckFileUrl} page={currSlide} maxHeight="600px" />
                    </div>
                ) : (
                    <div style={{
                        padding: 60,
                        ...
                        <div style={{ fontSize: 14, opacity: 0.8 }}>
                            {currentDeckId ? "PDF를 로딩 중입니다..." : "교시를 선택하거나 PDF를 업로드해주세요"}
                        </div>
                        {!currentDeckId && (
                            <button
                ...
                <div className="panel">
                    ...
                    {deckFileUrl ? (
                        <div className="pdf-stage">
                            <PdfViewer fileUrl={deckFileUrl} page={currSlide} maxHeight="400px" />
                        </div>
                    ) : (
                        <div style={{
                            padding: 40,
